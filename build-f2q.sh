...
DEFCONFIG="vendor/kona-not_defconfig vendor/samsung/kona-sec-not.config vendor/samsung/f2q.config"
...
make -j$(nproc --all) O=out ARCH=arm64 \
    ...
    LLVM=1 LLVM_IAS=1 dtbs   # ← dtbo.img 대신 dtbs

python3 scripts/mkdtboimg.py create "$BOOT_DIR/dtbo.img" --page_size=4096 \
    $(find "$BOOT_DIR/dts" -name "*.dtbo" | sort)
...
rm -rf AnyKernel3
echo "[*] Cloning AnyKernel3 for f2q"
